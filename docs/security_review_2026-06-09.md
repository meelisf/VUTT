...
| **K** — lost-update isikufailides | ✅ **parandatud** | per-isiku lukk `locks.py` `person_lock(id)` (jagatud ops + reciprocal_ops); katab `update_person`, `add_identifier`, `apply_enrichment`, `bulk_update_occupation`, `sync_reciprocals`; 3 testi sh 20-lõimeline lost-update (kinnitatud: ilma lukuta jääb 20-st alles 1–2). **Jääk:** `merge_person`/`delete_person` (admin-only, harv; merge teeb DB-ülese relations-ümberkirjutuse — eraldi teema) |
